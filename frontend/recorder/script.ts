...
function initAllHoldingSid(myData: any[]): void {
    for (let each of myData) allHoldingSids.add(each["sid"]);
}
...
function buildStockWarehouse(myData: any[]): void {
    ...
    for (let eachTradeRecord in myData) {
        ...
        let q = myData[eachTradeRecord]["deal-quantity"];
...
function buildRecordTable(myData: any[]): void {
    if (tradeRecordTableBody != null) {
        for (let eachRecord of myData) {
            ...
            crud.className = "crud";
            const btnConfigList = [
                { "btnClassName": "update-btn", "btnDisplayName": "更改", "cllbackFunc": readyToUpdTradeRecord, "args": {} },
                { "btnClassName": "delete-btn", "btnDisplayName": "刪除", "cllbackFunc": readyToDelTradeRecord, "args": {} }
            ];
...
function readyToUpdTradeRecord(e: Event): void {
...
function readyToDelTradeRecord(e: Event): void {
...
function cashInvChartData(endDateStr: string, myData: any[]): (string | number)[][] {
    let startDateStr = myData[myData.length - 1]["deal-time"].toString();
...
function getBalanceQForAllSids(): (string | number)[][] {
...
function componentChartData(balanceQForAllSids: (string | number)[][], stockInfoData: any[]): (string | number)[][] {
...
function showEachStockDetail(e: Event, sid: string, individualMktVal: number): void {
    ...
    applyIndividualCompareChart(eachStockCashInvst, individualMktVal);
}

function calcEachStockCashInvst(sid: string): number {
...
async function main(): Promise<void> {
    ...
    let componentData = componentChartData(balanceQForAllSids, stockInfoJson["data"]);
